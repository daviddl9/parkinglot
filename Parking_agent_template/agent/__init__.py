...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        for car in self.state.cars:
            ...
            res += ' (at pt{}pt{} {})'.format(car.position.x, car.position.y, car.id)
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="UP", parameters = (("from", "gridcell"), ("to", "gridcell")), precondition_string = "(and (at ?from agent1) (up_next ?from ?to) (not (blocked ?to)))", effect_string="(and (at ?to agent1) (not (at ?from agent1)))")
    gen.addAction(name="DOWN", parameters = (("from", "gridcell"), ("to", "gridcell")), precondition_string = "(and (at ?from agent1) (down_next ?from ?to) (not (blocked ?to)))", effect_string="(and (at ?to agent1) (not (at ?from agent1)))")
    gen.addAction(name="FORWARD", parameters = (("from", "gridcell"), ("to", "gridcell")), precondition_string = "(and (at ?from agent1) (forward_next ?from ?to) (not (blocked ?to)))", effect_string="(and (at ?to agent1) (not (at ?from agent1)))")
